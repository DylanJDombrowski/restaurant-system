...
export default function LocationsPage() {
  ...
  const registerTerminal = async (restaurant: Restaurant) => {
    try {
      ...
      setError(error instanceof Error ? error.message : "Failed to register terminal");
    } finally {
      setRegistering(null);
    }
  };
  ...
  const currentRegistration = typeof window !== "undefined" ? localStorage.getItem("pos_restaurant_id") : null;
  ...
  if (error) {
    return (
      <div className="text-center py-16">
        <div className="text-red-600 text-lg font-semibold mb-4">Error Loading Locations</div>
  ...
  return (
    <div className="space-y-8">
      {/* Page Header */}
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Terminal Registration</h1>
        <p className="text-gray-600 mt-2">
          Register this device as a POS terminal for a specific restaurant location. Once registered, this device will only be able to
          access that restaurant&apos;s data.
        </p>
      </div>
      ...
      {currentRegistration && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-lg font-semibold text-blue-900">Terminal Already Registered</h2>
          </div>
          <p className="text-blue-800 mb-3">
            This device is currently registered to: <strong>{localStorage.getItem("pos_restaurant_name")}</strong>
          </p>
          <p className="text-sm text-blue-700 mb-4">
            Registered on: {new Date(localStorage.getItem("pos_registered_at") || "").toLocaleString()}
          </p>
          <div className="flex gap-3">
            ...
            <button
              onClick={() => {
                if (
                  confirm("Are you sure you want to unregister this terminal? Staff will not be able to log in until it's re-registered.")
                ) {
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">Available Restaurant Locations</h2>
          <p className="text-sm text-gray-600 mt-1">Select the restaurant location where this device will be used</p>
        </div>
                ...
                <div className="flex-1">
                  ...
                  <div className="grid md:grid-cols-2 gap-4 text-sm text-gray-600">
                    <div>
                      ...
                      <code className="ml-2 bg-gray-100 px-2 py-1 rounded text-xs">{restaurant.id}</code>
                    </div>
                    <div>
                  ...
                  {currentRegistration && currentRegistration !== restaurant.id && (
                    <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded">
                      <p className="text-sm text-yellow-800">
                        ⚠️ Registering this device for a different restaurant will override the current registration. Staff using the
                        previous restaurant&apos;s PINs will no longer be able to log in on this device.
                      </p>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-gray-50 rounded-lg p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-3">How Terminal Registration Works</h3>
        <div className="space-y-2 text-sm text-gray-700">
          <p>
            <strong>1. Security:</strong> Each device can only access one restaurant&apos;s data
          </p>
          <p>
            <strong>2. Staff Access:</strong> Only staff assigned to the registered restaurant can log in
          </p>
          <p>
            <strong>3. Persistence:</strong> Registration survives browser restarts and app updates
          </p>
          <p>
            <strong>4. Admin Override:</strong> Only admins can change or remove registration
          </p>
        </div>
      </div>
    </div>
