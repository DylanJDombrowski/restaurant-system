...
export default function EnhancedStaffOrdersPage() {
  ...
  // MODIFIED/NEW: Centralized state for address selection and input fields
  ...
  const orderSummary = {
    ...
    tax: cartStats.tax,
    deliveryFee: orderType === "delivery" ? 3.99 : 0, // Example fee
    total:
      cartStats.subtotal +
      cartStats.tax +
  ...
  const lookupCustomer = useCallback(
    async (phone: string) => {
      if (phone.length < 10 || !restaurant) {
        ...
        // Do not call handleAddressSelection(null) here to avoid clearing fields
        // if the user is just correcting a typo in the phone number.
        // The useEffect for auto-selection will handle it if necessary.
        return;
      }
  ...
  const handleAddressSelection = useCallback(
    (addressId: string | null) => {
      ...
      if (addressId === null) {
        ...
        if (address) {
          ...
          // Should not happen if addressId comes from the list
          setDeliveryAddress({
            address: "",
            city: "",
            zip: "",
    ...
    [customerAddresses] // Dependency: re-create if customerAddresses changes
  );
  ...
  useEffect(() => {
    if (orderType === "delivery" && customerAddresses.length > 0) {
      const currentSelectedAddressIsValid = selectedAddressId
        ? customerAddresses.some((addr) => addr.id === selectedAddressId)
        : false;

      if (!currentSelectedAddressIsValid) {
        // Only auto-select if no valid address is already selected
        ...
        if (defaultAddress) {
          ...
          // No default/first address, ensure "new address" is selected
          handleAddressSelection(null);
        }
      }
    } else if (orderType === "pickup") {
      // If switching to pickup, and a saved address was selected,
      // set selectedAddressId to null to check "Enter new address".
      // The input fields are cleared by handleAddressSelection(null) if it's called.
      // To preserve field inputs if user toggles, just set ID to null.
      if (selectedAddressId !== null) {
        setSelectedAddressId(null);
        // Note: deliveryAddress fields will retain their values unless user explicitly clears them
        // or handleAddressSelection(null) is called, which now clears them.
        // For "pickup", we typically want the deliveryAddress fields to be irrelevant or cleared.
        // If you want to clear them when switching to pickup:
        // setDeliveryAddress({ address: "", city: "", zip: "", instructions: "" });
      }
    } else if (orderType === "delivery" && customerAddresses.length === 0) {
      // Delivery mode but no saved addresses, ensure "new address" is selected
      handleAddressSelection(null);
    }
  }, [customerAddresses, orderType, handleAddressSelection, selectedAddressId]);
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      const orderData = {
        ...
        customer_email: customerInfo.email || undefined, // Use undefined for Supabase
      ...
      const orderItemsData = cartItems.map((item) => ({
        ...
        unitPrice: item.totalPrice, // This is the configured unit price
      ...
      // Instead of reload, ideally update orders list if displayed on this page
      // For now, a success message and maybe redirect or refresh orders list
      alert(
        "Order created successfully! Order Number: " +
          responseData.data.order_number
      );
      // Potentially fetch recent orders again or navigate
      const ordersResponse = await fetch(
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        <div className="lg:col-span-3">
          <CustomerInformationPanel
            ...
            setCustomerInfoConfirmed={setCustomerInfoConfirmed}
            ...
            <div className="space-y-6">
              ...
              {cartItems.length > 0 && (
                <button
                  ...
                  className="w-full py-4 rounded-lg text-lg font-bold bg-green-600 text-white hover:bg-green-700 disabled:bg-green-400"
                >
                  {isSubmitting
                    ? "Creating Order..."
...
interface CustomerInformationPanelProps {
  ...
  >;
  selectedAddressId: string | null; // Prop from parent
  handleAddressSelection: (addressId: string | null) => void; // Prop from parent
  ...
  setCustomerInfoConfirmed: React.Dispatch<React.SetStateAction<boolean>>;
...
function CustomerInformationPanel(props: CustomerInformationPanelProps) {
  const {
    ...
    deliveryAddress,
    setDeliveryAddress,
    selectedAddressId, // Use prop from parent
    handleAddressSelection, // Use prop from parent
    isCustomerInfoExpanded,
    setIsCustomerInfoExpanded,
    customerInfoConfirmed,
    setCustomerInfoConfirmed,
      ...
      >
        <h2 className="text-xl font-semibold text-gray-900">
          {customerInfoConfirmed && !isCustomerInfoExpanded
            ? `Order for: ${customerInfo.name} (${customerInfo.phone}) - Type: ${orderType}`
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full border border-gray-300 rounded-lg px-3 py-2  text-stone-950 placeholder-gray-600   focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500  disabled:bg-gray-100 disabled:text-gray-500 disabled:cursor-not-allowed"
                    required
                ...
                customerAddresses.length > 0 && (
                  <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                    <p className="text-sm text-blue-800">
                      This customer has {customerAddresses.length} saved
                      address(es).
                    </p>
                    <button
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={`p-4 rounded-lg border-2 transition-all text-left ${
                    ...
                    customerInfoConfirmed ? "opacity-70 cursor-not-allowed" : ""
                  }`}
                >
              ...
              {orderType === "delivery" && (
                <div className="space-y-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                  ...
                  >
                    <input
                      type="radio"
                      name="savedAddress" // name should be consistent for radio group
                      value="new" // A non-ID value to signify new address
                      checked={selectedAddressId === null}
                      onChange={() =>
                        !customerInfoConfirmed && handleAddressSelection(null)
                      }
                      disabled={customerInfoConfirmed}
                      className="mr-2"
                    />
                  ...
                  <div className="grid grid-cols-1 gap-3 mt-2">
                    <input
                      ...
                      disabled={
                        customerInfoConfirmed ||
                        (selectedAddressId !== null && orderType === "delivery")
                      }
                    />
                    <div className="grid grid-cols-2 gap-3">
                      <input
          ...
          {!customerInfoConfirmed && (
            <div className="pt-4 border-t border-gray-200">
              <button
                ...
                className="w-full bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-semibold text-lg transition-colors disabled:bg-gray-400"
                disabled={
                  !customerInfo.name ||
                  !customerInfo.phone ||
                  (orderType === "delivery" &&
