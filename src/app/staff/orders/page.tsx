...
export default function StaffOrdersPage() {
  ...
  const [selectedAddress, setSelectedAddress] = useState<CustomerAddress | undefined>(undefined); // ✅ NEW

  const [orderType, setOrderType] = useState<"pickup" | "delivery">("pickup");

  // Order completion modal states
  const [showOrderCompletionModal, setShowOrderCompletionModal] = useState(false);
  const [pendingLoyaltyRedemption, setPendingLoyaltyRedemption] = useState<LoyaltyRedemption | null>(null);
  ...
  const orderSummary = {
    ...
    total: cartStats.subtotal + cartStats.tax + (orderType === "delivery" ? 3.99 : 0) - (pendingLoyaltyRedemption?.discount_amount || 0),
  };
  ...
  const handleCustomerSelected = (customer: CustomerLoyaltyDetails | null, address?: CustomerAddress) => {
    setCustomer(customer);
    if (address) {
      setSelectedAddress(address); // ✅ Set the address here
      console.log("🏠 Selected customer with default address:", address.label, address.street);
    } else {
  ...
  const handleOrderCompletionConfirm = async (orderData: {
    ...
    try {
      const finalLoyaltyDiscount = orderData.loyaltyRedemption?.discount_amount || 0;
      const finalTotal = Math.max(0, orderSummary.subtotal + orderSummary.tax + orderSummary.deliveryFee - finalLoyaltyDiscount);
      ...
      const orderPayload = {
        ...
        ...(orderData.orderType === "delivery" &&
          orderData.deliveryAddress && {
            ...
            delivery_instructions: orderData.deliveryAddress.instructions || undefined,
          }),
      };
          ...
          <div className="px-4 md:px-6 py-4">
            <div className="flex justify-between items-center mb-4">
              <div>
                ...
                <div className="flex items-center gap-4 text-sm text-gray-900 mt-1">
                    ...
                    <>
                      ...
                      {pendingLoyaltyRedemption && (
                        <span className="text-purple-600 font-medium">
                          🎁 -{pendingLoyaltyRedemption.discount_amount.toFixed(2)} loyalty
                        </span>
                      )}
                    </>
                  )}
            ...
            <div className="flex space-x-1 bg-gray-100 p-1 rounded-lg w-fit">
              <button
                ...
                className={`px-6 py-2 rounded-md font-medium transition-all ${
                  activeTab === "new-order" ? "bg-blue-600 text-white shadow-sm" : "text-gray-900 hover:text-gray-900 hover:bg-white"
                }`}
              >
                📝 New Order
                {cartItems.length > 0 && (
              ...
              <button
                ...
                className={`px-6 py-2 rounded-md font-medium transition-all ${
                  activeTab === "pickup" ? "bg-orange-600 text-white shadow-sm" : "text-gray-900 hover:text-gray-900 hover:bg-white"
                }`}
              >
                🍕 Ready for Pickup
          ...
          {activeTab === "new-order" ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
              ...
              <div className="lg:col-span-2">
                <MenuNavigator menuItems={menuItems} onAddToCart={handleAddToCart} restaurantId={restaurant?.id || ""} />
              </div>

              {/* RIGHT: Cart & Customer Info */}
              <div className="space-y-4">
                {/* ✅ UPDATED: Customer Details with address passing */}
                <CustomerDetails onCustomerSelected={handleCustomerSelected} restaurantId={restaurant?.id || ""} />
  ...
  if (orders.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
        ...
        <p className="text-gray-900 mb-6">No orders are currently ready for pickup.</p>
        <div className="text-sm text-gray-900">Orders will appear here when the kitchen marks them as ready.</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm text-gray-900">Click Mark as Picked Up when customer collects their order</div>
      </div>
            ...
            >
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <div className="text-lg font-medium text-gray-900">{order.customer_name}</div>
                </div>
              ...
              <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                ...
                {order.customer_email && <div className="text-sm text-gray-900">✉️ {order.customer_email}</div>}
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="text-xs text-gray-900 mt-1">Ordered at {new Date(order.created_at).toLocaleTimeString()}</div>
              </div>
