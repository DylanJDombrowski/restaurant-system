...
import { PizzaCustomizationResult, PizzaCustomizationsByCategory, PizzaToppingSelection, PizzaValidationResult } from "@/lib/types/pizza";
...
export function usePizzaCustomization(
  ...
  const availableCustomizations: PizzaCustomizationsByCategory = {
    toppings: allCustomizations.filter((c) => c.category.startsWith("topping_") && c.applies_to.includes("pizza")),
    sauces: allCustomizations.filter((c) => c.category === "topping_sauce" && c.applies_to.includes("pizza")),
    preparation: allCustomizations.filter((c) => c.category === "preparation" && c.applies_to.includes("pizza")),
  };
  ...
  const defaultSelections: ConfiguredModifier[] = getPizzaDefaultSelections(variant, availableCustomizations);

  // Pizza price calculation - FIXED: No any types
  const calculatePrice = async (selectedToppings: PizzaToppingSelection[]): Promise<number> => {
    try {
  ...
  const validate = (selectedToppings: PizzaToppingSelection[]): PizzaValidationResult => {
    const errors: string[] = [];
    ...
    const hasSauce = selectedToppings.some((topping) => {
      const customization = availableCustomizations.sauces.find((s) => s.id === topping.customization_id);
...
function getPizzaDefaultSelections(variant: MenuItemVariant, availableCustomizations: PizzaCustomizationsByCategory): ConfiguredModifier[] {
  const defaults: ConfiguredModifier[] = [];

  // Default sauce for regular pizzas
  const defaultSauce = availableCustomizations.sauces.find(
    (s) => s.name.toLowerCase().includes("marinara") || s.name.toLowerCase().includes("red")
  );
